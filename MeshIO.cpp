...
void MeshIO<IndexType, ValueType>::createRandomStructured3DMesh_dist(CSRSparseMatrix<ValueType> &adjM, std::vector<DenseVector<ValueType>> &coords, std::vector<ValueType> maxCoord, std::vector<IndexType> numPoints){
    ...
    // create a structured mesh first, then change it
    ...
    IndexType ngbUpperBound = std::min(12, indexCnt); // I do not know, just trying 12
    IndexType ngbLowerBound = 3;
    srand(time(NULL));
    
    CSRStorage<ValueType>& localStorage = adjM.getLocalStorage();
    
    scai::utilskernel::LArray<IndexType> ia = localStorage.getIA();
    scai::utilskernel::LArray<IndexType> ja = localStorage.getJA();
    scai::utilskernel::LArray<ValueType> values = localStorage.getValues();
