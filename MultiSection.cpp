...
template<typename IndexType, typename ValueType>
std::vector<std::shared_ptr<rectCell<IndexType,ValueType>>> MultiSection<IndexType, ValueType>::getPartitionNonUniform( 
    ...
    std::vector<std::vector<ValueType>> scaledCoords(dim);
...
PRINT0(scale);        
        for (IndexType d = 0; d < dim; d++) {
            //get local parts of coordinates
            const scai::utilskernel::LArray<ValueType>& localPartOfCoords = coordinates[d].getLocalValues();
            scaledCoords[d].assign( localPartOfCoords.size() , 0 );
            for (IndexType i = 0; i < localN; i++) {
                ...
                SCAI_ASSERT( i<scaledCoords[d].size() , "Too large index");
                scaledCoords[d][i] = scaledCoord;
    ...
    for (IndexType d=0; d<dim; d++) {
        ...
        if( scaledMin[d]!=0 ){
            ...
            //      elements will just me always 0.
            throw std::logic_error("Minimum scaled value should be 0 but it is " + std::to_string(scaledMin[d]) );
        }
    }
    SCAI_ASSERT( localN==scaledCoords[0].size(), "Wrong size of scaled coordinates vector.");
    ...
    // at first the bounding box is the whole space
    for(int d=0; d<dim; d++){
        bBox.bottom.push_back( scaledMin[d] );
        bBox.top.push_back( scaledMax[d] );
    }
...
PRINT0(*thisDimCuts);
        ...
        // a vector of size numLeaves. projections[i] is the projection of leaf/rectangle i in the chosen dimension
        std::vector<std::vector<ValueType>> projections = MultiSection<IndexType, ValueType>::projectionNonUniform( scaledCoords, nodeWeights, root, chosenDim, settings);
...
std::vector<std::vector<ValueType>> MultiSection<IndexType, ValueType>::projectionNonUniform( 
    ...
    // calculate projection for local coordinates
    {
        ...
        for(int i=0; i<localN; i++){
            SCAI_REGION_START("MultiSection.projection.localProjection.indexAndCopyCoords");
            const IndexType globalIndex = inputDist->local2global(i);
            std::vector<ValueType> coords = coordinates[i];
            //TODO: avoid the conversion to vector<double>
            //std::vector<ValueType> coordsVal( coords.begin(), coords.end() );
            SCAI_REGION_END("MultiSection.projection.localProjection.indexAndCopyCoords");
...
PRINT(*comm << ": localMax= "<< maxCoords[dim] << " , localMin=" << minCoords[dim]);        
        }
...
template std::vector<std::shared_ptr<rectCell<int,double>>> MultiSection<int, double>::getPartitionNonUniform( const scai::lama::CSRSparseMatrix<double> &input, const std::vector<scai::lama::DenseVector<double>> &coordinates, const scai::lama::DenseVector<double>& nodeWeights, Settings settings);

template std::vector<std::vector<double>> MultiSection<int, double>::projectionNonUniform( const std::vector<std::vector<double>> &coordinates, const scai::lama::DenseVector<double>& nodeWeights, const std::shared_ptr<rectCell<int,double>> treeRoot, const std::vector<int>& dimensionToProject, Settings settings);
