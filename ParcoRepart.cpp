...
ValueType ITI::ParcoRepart<IndexType, ValueType>::twoWayLocalFM(const CSRSparseMatrix<ValueType> &input, const CSRStorage<ValueType> &haloStorage,
	...
	while (firstQueue.size() + secondQueue.size() > 0 && iterWithoutGain < magicStoppingAfterNoGainRounds) {
		...
		if (firstQueue.size() == 0) {
			...
			if (gainOverBalance) {
				firstComparisonPair = {-firstQueue.inspectMin().first, blockSizes.first};
				firstComparisonPair = {-secondQueue.inspectMin().first, blockSizes.second};
			} else {
				firstComparisonPair = {blockSizes.first, -firstQueue.inspectMin().first};
				secondComparisonPair = {blockSizes.second, -secondQueue.inspectMin().first};
			}
