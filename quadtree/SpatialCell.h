...
template <class T>
class SpatialCell : public std::enable_shared_from_this<SpatialCell<T>> {
...
//-------------------------------------------------------------------------------------------
	...
	scai::lama::CSRSparseMatrix<ValueType> getSubTreeAsGraph() {
            ...
            std::vector< std::set<std::shared_ptr<SpatialCell<T>>>> graphNgbrsCells( treeSize );
    ...
    ////// not sure if this is the right way. At least it works for now
            std::vector<std::shared_ptr<SpatialCell<T>>> frontier;
        ...
        //PRINT(frontierI<< ": thisNode ID: " << thisNode->getID()); 
                ...
                for(unsigned int c=0; c<thisNode->children.size(); c++){
                    ...
                    for(unsigned int s=c+1; s<thisNode->children.size(); s++){
                        std::shared_ptr<SpatialCell<T>> sibling = thisNode->children[s];
                    ...
                    for(typename std::set<std::shared_ptr<SpatialCell<T>>>::iterator graphNgb= graphNgbrsCells[thisNode->ID].begin(); graphNgb!=graphNgbrsCells[thisNode->getID()].end(); graphNgb++){
            ...
            if( !thisNode->isLeaf ){
                ...
                //std::shared_ptr<SpatialCell<T>> la(thisNode);
                // first remove this->ID from others
                for(typename std::set<std::shared_ptr<SpatialCell<T>>>::iterator graphNgb= graphNgbrsCells[thisNode->getID()].begin(); graphNgb!=graphNgbrsCells[thisNode->getID()].end(); graphNgb++){
           
                    assert( graphNgb->get()->getID() < graphNgbrsCells.size());
                    std::set<std::shared_ptr<SpatialCell<T>>>& ngbSet = graphNgbrsCells[graphNgb->get()->getID()];
                    
                    typename std::set<std::shared_ptr<SpatialCell<T>>>::iterator fnd= ngbSet.find(std::shared_ptr<SpatialCell<T>>(thisNode) ) ;

PRINT(ngbSet.size()<<" __ "<< fnd->get()->getID() );     

                    std::shared_ptr<SpatialCell<T>> found;
                    if( fnd != ngbSet.end() ){
...
PRINT( nnzValues << " <> "<< countLeaves() );            
            ...
            
            {
                ...
                for(IndexType i=0; i<graphNgbrsCells.size(); i++){
                    ...
                    // graphNgb is a neighbouring cell of this node as a shared_ptr
                    for(typename std::set<std::shared_ptr<SpatialCell<T>>>::iterator graphNgb= graphNgbrsCells[i].begin(); graphNgb!=graphNgbrsCells[i].end(); graphNgb++){
...
PRINT(csrIA.size() << " _ " << csrJA.size() << " @ " << csrValues.size() );                
        ...
        static int getVSsize(std::vector< std::set<std::shared_ptr<SpatialCell<T>>>> input){
            int size= 0;
            for(int i=0; i<input.size(); i++){
                for(typename std::set<std::shared_ptr<SpatialCell<T>>>::iterator graphNgb= input[i].begin(); graphNgb!=input[i].end(); graphNgb++){
                    ++size;
        ...
        bool isAdjacent(SpatialCell<T>& other){
