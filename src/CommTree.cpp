...
//h = hierarchyLevels-1 because we starting counting from 0
	for(unsigned int h = hierarchyLevels-1; h>=0; h--){
...
PRINT("level below has size " << levelBelowsize );
	std::vector<commNode> aboveLevel( aboveLevelSize );
	...
	for( unsigned int i=0; i<levelBelowsize; i++){
		...
		//distinctHierPrefixes.push_bak(thisPrefix);
...
PRINT("Size of level above (lvl " << h << ") is " << aboveLevelSize );
